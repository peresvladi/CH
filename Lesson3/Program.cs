...
/* 
void Method2_1 (String msg) // имя (индефикоатор) М.
    {
...
Console.WriteLine(year); // Вывод на экран значения через переменную 
*/
...
// 3. Повторять 1 и 2 пока есть несортированные элементы
            ...
            // НАЧАЛО
int []arr = {1,5,4,3,2,6,7,1,1}; //  объявляем массив заполняя его 
//  неотсортироваанными элементами
...
void PrintArray(int[]array)
{
    ...
    for(int i = 0; i < count; i++) // создаем цикл которые работает пока 
    // переменная count больше обявленной в нем переменной i которой при 
    // объявлении присвоено нулевое значение и установлен инкримент 
    // (проходит циклом по элементам массива определяя их для дальнейшего 
    // вывода на экран)
    {
     ...
     void SelectionSort(int[] array) //Создаем метод который упорядочивает 
     // элементы массива. Аркгументом устанавливаем array (некий массив).

     {
        for(int i = 0; i < array.Length -1; i++) // с помомщью цикла 
        // считываем значения элементов массива
         
        {
           ...
           for(int j = i + 1; j < array.Length; j++) // ищем минимальный элемент           
            {
            if (array[j] < array [minPosition]) minPosition = j; // ищем и 
            // сохраняем минимальный элемент
            }
            int temporari = array[i]; // присваиваем значение i временной 
            //  переменной temporary, что бы не утратить его при дальнейшем
            // обмене значениями с элементом массива имеющем индекс 
            // minPosition
            array[i] = array[minPosition]; // изменяем на 1 индекс элемента i, 
            // уменьшая на 1 неотсортированную часть массива при дальнейшем 
            // исполнении кода (до завершения колличества всех элементов)
